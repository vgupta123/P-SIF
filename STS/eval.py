...
def sim_getCorrelation(We,words,f, weight4ind, scoring_function, params):
    ...
    for i in lines:
            i = i.split("\t")
            #print(i)
            p1 = i[0].lower(); p2 = i[1].lower(); score = float(i[2])
...
def sim_getCorrelation1(We,words,file_index, weight4ind, scoring_function, params):
...
def sim_evaluate_one1(print_line,file_index,We, words, weight4ind, scoring_function, params):
    prefix = "../data/"
    parr = []; sarr = []
    #pdb.set_trace()
    farr = [file_index[0].split("/")[-2]+file_index[0].split("/")[-1]]

    for i in farr:
        p = sim_getCorrelation1(We, words, file_index, weight4ind, scoring_function, params)
        parr.append(p);
