...
def prob_wordvecs_parallel(param_each):
    ...
    prob_wordvecs_name = "data_files/prob_wordvecs_gmm_"+textfile.split("/")[-1]+"_all_words_"+str(n_clusteri)+".pkl"
    pickle.dump(prob_wordvecs,open(prob_wordvecs_name,"wb"))
    print(str(n_clusteri) +"_done")

parameters = pd.read_csv("parameters_gmm.csv",delimiter="\t")
cluster_coef_pairs = parameters[["ksvd clusters"]].values.tolist()
