...
model = pickle.load(open("word_embedding_dict.pkl","r"))
# Get wordvectors for all words in vocabulary.
word_vectors = np.array(model.values())
...
for num_clusters in [40]:#,40]:#,40]:
  ...
  for _ in range(5):
    ...
    # a cluster number
    word_centroid_map = dict(zip(model.keys(), idx))
    # Create a Word / Probability of cluster assignment dictionary, mapping each vocabulary word to
    # list of probabilities of cluster assignments.
    word_centroid_prob_map = dict(zip(model.keys(), idx_proba))
