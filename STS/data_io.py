...
def getWordmap_ksvd(prob_wordvecs_ksvd,list_words_set):

    words={}
    We = []
    n = 0
    # prob_wordvecs_ksvd.item().get(word)
    for word, embeding in prob_wordvecs_ksvd.iteritems():
        # print(n)
        if(word in list_words_set):
            We.append(list(embeding))
            words[word]=n
            n+=1
    print("ok")
    return (words, np.array(We))

def getWordmap_ksvd_supervised(textfile):
    prob_wordvecs_ksvd = np.load(textfile)
    words={}
    We = []
    n = 0
    # prob_wordvecs_ksvd.item().get(word)
    for word, embeding in prob_wordvecs_ksvd.item().iteritems():
            We.append(list(embeding))
            words[word]=n
            n+=1
    print("ok")
